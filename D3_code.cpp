...
extern const unsigned char epd_bitmap_up_right[]= {
	0x0f, 0xff, 0x0f, 0xfe, 0x0f, 0xff, 0x0f, 0xfe, 0x0f, 0xff, 0x0f, 0xfe, 0x0f, 0xff, 0x0f, 0xfe, 
	0x0f, 0xff, 0x0f, 0xfe, 0x0f, 0xff, 0x0f, 0xfe, 0x0f, 0xff, 0x0f, 0xfe, 0x0f, 0xff, 0x0f, 0xfe, 
...
void parsePacket(String data) {
  // Example: "MAC,,HEADING" or "SOS,MAC"
  ...
  String headingStr = data.substring(secondComma + 1);
  float heading = headingStr.toFloat();

  for (int i = 0; i < NUM_PEERS; i++) {
    if (peers[i].mac == mac) {
      peers[i].heading = heading;
...
WiFiServer subServer(12346);  // Different port for D4
...
void setup() {
  ...
  pChar = pService->createCharacteristic("ABCD", BLECharacteristic::PROPERTY_READ);
  pChar->setValue("D3 BLE Active");
  pService->start();
...
void loop() {
  ...
  pChar->setValue(String(selfHeading).c_str());
...
if (client.connected()) {
  ...
  // ⬇️ Add this block immediately after sending
...
while (client.available()) {
  String data = client.readStringUntil('\n');
  parsePacket(data); // Handle D1's response
}
  ...
  WiFiClient subClient = subServer.available();
  if (subClient && subClient.connected()) {
    String data = subClient.readStringUntil('\n');
    // Parse D4's heading from data and update peers[1].heading
    if (data.length() > 0) {
      // Example: "68:25:DD:33:B2:70,123"
      int commaIdx = data.indexOf(',');
      if (commaIdx > 0) {
        String mac = data.substring(0, commaIdx);
        float heading = data.substring(commaIdx + 1).toFloat();
        if (mac == peers[1].mac) {
          peers[1].heading = heading;
        }
      }
      // Handle SOS
      if (data.startsWith("SOS")) {
