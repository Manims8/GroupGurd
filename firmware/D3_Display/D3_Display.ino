/*
 * =================================================================
 * D3 - PERIPHERAL DEVICE (WITH DISPLAY) - WI-FI VERSION (MODIFIED)
 * =================================================================
 *
 * Role:
 ...
 * 5. On warning: vibrates and displays alert on OLED.
 *
...
DeviceState d1_state = {"D1", -1.0, -1, 0, false};
DeviceState d4_state = {"D4", -1.0, -1, 0, false};
int hed;
...
void setup() {
  ...
  if (!display.begin(SSD1306_SWITCHCAPVCC, 0x3C)) {
    Serial.println(F("D3: SSD1306 allocation failed"));
    for (;;);
  }
...
void loop() {
  ...
  if (warningReceived) {
    Serial.println("[D3] Warning received!");
    showingD1Alert = true;
  ...
  if (millis() - lastSendTime > sendInterval) {
    mySosActive = (digitalRead(BUTTON_PIN) == HIGH);
    if (mySosActive) {
      Serial.println("[D3] SOS ACTIVE!");
      triggerAlert(2000, 200, 200);
    }
...
void connectToWiFi() {
  ...
  if(WiFi.status() == WL_CONNECTED){
    Serial.println("\nWiFi Connected.");
    Serial.print("D3 IP address: "); Serial.println(WiFi.localIP());
  }
}
...
void handleIncomingData() {
  ...
  if (client) {
    ...
    Serial.print("D3 Received Data: "); Serial.println(data);

    if (data == "WARN") {
      warningReceived = true;
    } else if (data.startsWith("D1_DATA")) {
      char data_cstr[64];
      data.toCharArray(data_cstr, 64);
      char* token = strtok(data_cstr, ","); // Skip D1_DATA
      
      token = strtok(NULL, ","); if(token) d1_state.heading = atoi(token);
      token = strtok(NULL, ","); if(token) d1_state.distance = atof(token);
      d1_state.last_seen = millis();
      
      // Parse D4 data forwarded by D1
      token = strtok(NULL, ","); if(token) d4_state.heading = atoi(token);
      token = strtok(NULL, ","); if(token) d4_state.distance = atof(token);
      bool previousD4Sos = d4_state.sos_active;
      token = strtok(NULL, ","); if(token) d4_state.sos_active = (bool)atoi(token);
      if (!previousD4Sos && d4_state.sos_active) {
        triggerAlert(3000, 300, 100);
      }
      token = strtok(NULL, ","); if(token) {
        if (atoi(token) == 1) {
          d4_state.last_seen = millis();
        } else {
          d4_state.last_seen = 0;
        }
      }
    }
  }
...
void updateDisplay() {
    ...
    if (mySosActive || d4_state.sos_active) {
        if (mySosActive && d4_state.sos_active) drawSOSOverlay("SOS ALL", true);
        else if (mySosActive) drawSOSOverlay("SOS", true);
        else drawSOSOverlay("SOS D4", false);
        display.display();
        return;
    }

    if (d1_state.distance > DISTANCE_THRESHOLD && d1_state.last_seen > 0) {
        drawRangeWarning("D1 OUT RANGE", d1_state.distance);
        display.display();
        return;
    }
    if (d4_state.distance > DISTANCE_THRESHOLD && d4_state.last_seen > 0) {
        drawRangeWarning("D4 OUT RANGE", d4_state.distance);
        display.display();
        return;
    }
    
    // NEW LAYOUT: Split screen showing D1 and D4 side-by-side
    ...
    drawPeerBox(margin, margin, boxW, boxH, d1_state);
    drawPeerBox(midX + margin, margin, boxW, boxH, d4_state);

    display.display();
}

// Renders content for one peer in a specific box area on D3's screen
void drawPeerBox(int16_t x, int16_t y, int16_t w, int16_t h, const DeviceState &p) {
    ...
    const int16_t arrowH   = 30; // 32x30 arrow
    ...
    char buf[8];
    dtostrf(p.distance, 3, 1, buf); // Format as " 1.2"
